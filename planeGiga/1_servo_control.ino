...
void updateStepperLogic()
{
  ...
  if (fabs(roll) < ROLL_DEADZONE_DEG && abs(effectiveYaw) < 5)
    return;
  ...
  if (currentStepPos >= MAX_STEP_POS && turnDrive > 0)
    return;
  if (currentStepPos <= MIN_STEP_POS && turnDrive < 0)
    return;
  ...
  // Exponential curve: t normalised 0..1, then (e^(k*t) - 1) / (e^k - 1)
  // At small angles t≈0 → nearly 0. At full angle t=1 → exactly 1.
  // EXP_STEEPNESS controls how aggressively it ramps up.
